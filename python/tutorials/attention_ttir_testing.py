...
if __name__ == "__main__":
    ...
    BLOCK_N=64
    H = N_HEADS
    q = numpy.zeros((BATCH, H, N_CTX, HEAD_DIM), dtype=numpy.float16)
    k = numpy.zeros((BATCH, H, N_CTX, HEAD_DIM), dtype=numpy.float16)
    v = numpy.zeros((BATCH, H, N_CTX, HEAD_DIM), dtype=numpy.float16)
    sm_scale = 0.5
    o = numpy.zeros((BATCH, H, N_CTX, HEAD_DIM), dtype=numpy.float16)
    grid = lambda args: (triton.cdiv(q.shape[2], args["BLOCK_M"]), q.shape[0] * q.shape[1], 1)
    M = numpy.empty((q.shape[0], q.shape[1], q.shape[2]), dtype=numpy.float16)
    stage=1

    types = ((["*fp16"] * 3) + ["fp32"] + (["*fp16"] * 2) + (["i32"] * 19) + (["constexpr"] * 4))
    signature = {k: v for (k, v) in zip(_attn_fwd.arg_names, types)}
    ...
    backend = tc.make_backend(target)
    context = tc.make_context(backend)
    extra_options = src.parse_options()
    options = backend.parse_options(dict(), **extra_options)
    ttir = src.make_ir(target, options, backend.get_codegen_implementation(options), backend.get_module_map(), context)
    print(ttir)
